...
describe('blog entity', () => {
  ...
  describe('Blog POST create', () => {
    ...
    it('should return 401 if no authorization header', async () => {
      await request(app).post(BLOGS_PATH).send(validBlogDto).expect(401);
    });
  });
  ...
  describe('Blog POST->GET :id', () => {
    it('should return blog by id', async () => {
      ...
      const resGet = await request(app).get(`${BLOGS_PATH}/${resBlogEntity.body.id}`).expect(200);
    ...
    it('should return 404 if blog does not exist', async () => {
      await request(app).get(`${BLOGS_PATH}/999999999999999999999999`).expect(404);
    });
  });
  ...
  describe('Blog PUT', () => {
    it('POST ->PUT ->GET should update blog and return 204', async () => {
      ...
      const res = await request(app).get(`${BLOGS_PATH}/${createdBlogId}`).expect(200);
  ...
  describe('Blog GetList', () => {
    it('should return paginated blogs list with default params', async () => {
      ...
      const res = await request(app).get(BLOGS_PATH).expect(200);
    ...
    it('should filter blogs by searchNameTerm', async () => {
      ...
      const res = await request(app).get(BLOGS_PATH).query({ searchNameTerm: 'Mail' }).expect(200);
  ...
  describe('Blog , create PostEntity BlogPOST -> BlogPostPOST ', () => {
    it('should return 201 create post by id', async () => {
      ...
      const res = await request(app).get(`${POSTS_PATH}/${postId}`).expect(200);
    ...
    it('should return 401 if no authorization header', async () => {
      ...
      await request(app).post(`${BLOGS_PATH}/${validButNotExistingBlogId}/posts`).expect(401);
    });
  ...
  describe('Blog getAll Posts for Blog', () => {
    ...
    it('should return 404 if blog does not exist', async () => {
      ...
      await request(app).get(`${BLOGS_PATH}/${validButNotExistingBlogId}/posts`).expect(404);
    });
  });
});
