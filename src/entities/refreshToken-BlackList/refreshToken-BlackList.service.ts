...
export const refreshTokenBlackListService = {
    ...
    refreshTokens: async (token:string):Promise<ResultType<null| {accessToken:string,refreshToken:string}>> => {

        const { id }:JwtPayload  =  jwt.verify(token, SETTINGS.JWT_REFRESH_SECRET) as {id:string}

        const addedTokenToBlackList:boolean = await refreshTokenBlackListRepository.addInvalidToken(token,id)
